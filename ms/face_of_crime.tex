...
The UCR data on race of alleged criminals is available for 1995--2015. In UCR, race is split into white, black, American Indian or Alaskan Native, Asian, and Native Hawaiian or Other Pacific Islander. The totals in these categories include Hispanics. Data on Hispanic background only began to be collected in 2013, and even then the data doesn't split race by Hispanic ethnicity. These raw numbers are likely severely biased---nearly 93\% of Hispanics self-identify as Whites and pooling Hispanics likely inflates the proportion of White criminals more than Blacks \citep{steffensmeier2011reassessing}. To arrive at commensurable numbers, we exploit the method proposed by \citet{steffensmeier2011reassessing}---we adjust UCR data by leveraging data from states (California and New York) where arrest data includes Hispanics. We use these adjusted numbers when making comparisons.
...
Victim data from the NCVS covers the period between 1993--2014. Racial data in the NCVS can be gathered with Hispanics included in the category totals, and without. We collected both kinds of data, but focus on data that more closely matches our data, and separates out Hispanics.
